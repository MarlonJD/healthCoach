...
- Signed Xcode builds passed with the installed Xcode 27.0 toolchain and locally selected Apple Development identity: `HealthCoachIOS` for generic iOS, `HealthCoachMac` for generic macOS, and `HealthCoachWatch` for generic watchOS. The Mac build ran its real helper build phase and placed executable `healthcoach-mcp` at `HealthCoachMac.app/Contents/Helpers/healthcoach-mcp`; the app also passed deep code-sign verification.
