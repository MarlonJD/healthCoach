...
- Signing-disabled Simulator builds passed for the iOS and watchOS schemes. The iPhone Simulator installed and launched `com.marlonjd.HealthCoach` from the fresh build, and a screenshot confirmed the redesigned Today cards, automatic-body-data presentation, and meal quick-add UI rendered. The Apple Watch Simulator installed and launched `com.marlonjd.HealthCoach.watchkitapp` from the fresh standalone build, and a screenshot confirmed the expected cached-workout/iPhone-unavailable UI rendered. The temporary Simulators were then shut down.
