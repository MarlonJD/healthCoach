...
Status: independently reviewed; six findings repaired. User-directed live Watch workout expansion incorporated 2026-09-16.
...
The user first requested a modest Watch app and subsequently explicitly expanded it to include a real live workout. Add a SwiftUI companion that shows the cached accepted workout/day, current/next exercise, and set targets; allows session start, quick set/reps/kg entry (optional RIR), and session completion; displays pending/synced/error state; and, while active, shows current/average/peak heart rate, active energy, and elapsed time from a real `HKWorkoutSession` with its associated `HKLiveWorkoutBuilder`/`HKLiveWorkoutDataSource`. The Watch requests HealthKit authorization in context, uses `WKBackgroundModes = workout-processing`, and reconnects an interrupted active session through `handleActiveWorkoutRecovery`. The system workout is saved to HealthKit only after the user finishes it. HealthCoach persists a bounded final summary in the canonical phone session and sends that session through the existing Mac outbox. Wrist temperature is intentionally excluded. Do not add an independent Mac/Codex connection, Watch meals, complications, or widgets.

Use WatchConnectivity with the paired iPhone: `updateApplicationContext` for small replaceable workout/session snapshots carrying monotonic phone revisions, and `transferUserInfo` for ordered commands and application acknowledgments. Activate/check the session and installed counterpart before transfer. Watch commands (`startSession`, `recordSet`, `finishSession`) have UUIDs, session IDs, per-session sequence, exercise IDs, original program revision, and typed values; the finish command carries the validated bounded live-workout summary. Persist the queue/cache on Watch before showing a save; phone validates, deduplicates, applies into its canonical store, and enqueues existing Mac sync in one transaction before acknowledging. Start is persisted only after the real HealthKit session starts; finish saves the HealthKit workout before its durable phone command is queued, and the phone applies it only after preceding commands commit. Watch removes acknowledged commands only; transport delivery alone is insufficient. Complete WatchConnectivity background tasks promptly and show eventual delivery honestly.
...
Build the Watch target and test queue restart, duplicate delivery, session ordering/completion, old-program logging, live-summary validation, and error retention with deterministic fixtures. Real HealthKit authorization/sensor values, workout background recovery, and `transferUserInfo` delivery/background behavior require paired physical devices; the Simulator does not support `transferUserInfo`, so label those checks `not run` when devices are unavailable.
...
4. **Training, coaching, and Watch companion.** Add profile constraints, equipment, set logging, personal exercise library, program generation/acceptance, alternatives, progression and questions; then add the cached Watch interface/WatchConnectivity flow and real HealthKit workout telemetry above. Acceptance: the complete proposed program survives restart, its cached alternatives work offline, and late/stale results cannot replace a correction/current program. Watch set capture survives restart, command retry does not duplicate sets, a real Watch workout displays live heart rate/active energy and saves on finish, and start/set/finish plus its bounded summary becomes one complete phone-side session.
...
- Real Apple Watch HealthKit authorization, sensor readings, workout-processing background/recovery behavior, and paired WatchConnectivity transfer require physical-device verification. Record `not run` if unavailable. Do not call a simulator run a real-device pass.
...
- [Running workout sessions](https://developer.apple.com/documentation/healthkit/running-workout-sessions), [HKLiveWorkoutDataSource](https://developer.apple.com/documentation/healthkit/hkliveworkoutdatasource), and [WKBackgroundModes](https://developer.apple.com/documentation/bundleresources/information-property-list/wkbackgroundmodes): Watch workout lifecycle, live heart-rate/energy collection, and workout-processing background execution.
