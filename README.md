...
The system deliberately has no application backend, cloud database, CloudKit, account system, custom OpenAI HTTP client, nutrition catalog, large exercise catalog, ML engine, or recommendation engine. Codex supplies meal estimates with uncertainty, complete training programs with cached alternatives, progression proposals, equipment findings, and holistic answers from the bounded local context exposed by MCP. HealthCoach owns state, validation, synchronization, deterministic summaries, and UI.
...
On the Mac, open the menu bar companion and scan its short-lived QR code from the iPhone Settings tab. The QR contains a random 32-byte pairing secret, pair/peer identifiers, protocol version, and expiry—not health content. The Mac automatically replaces a pending QR shortly before expiry and never lets an older in-flight handshake activate after rotation. The secret is stored in the device-only Keychain. Bonjour advertises only the HealthCoach service and Mac peer identifier. Sync uses a Network.framework TLS pre-shared key with the pairing identity hint, length-bounded frames, authenticated peer IDs, durable two-way outboxes, cursors, and staged snapshot bootstrap. Interrupted bootstrap is discarded and retried without exposing partial state.

The iPhone reconnects opportunistically while the app is foregrounded; the Mac listener remains available while the companion runs and can be enabled at login. When the Mac commits a job status or result, the active sync session is woken and sends the durable Mac outbox immediately; if no socket is available, the next foreground reconnect retries it. iOS background execution and suspended LAN delivery remain subject to Apple's scheduling limits. The UI reports pending, synced, disconnected, and error states instead of treating a queued mutation as delivered.

## Codex and MCP boundary

Equipment discovery is the only job allowed to opt into public web lookup or image input, and only after the user enables the facility-lookup setting. Health and normal program jobs remain local-context, read-only, and network-disabled.
...
The Mac companion keeps a lightweight durable queue monitor while it is running and dispatches up to two independent Codex jobs concurrently. Transient failures retry with capped backoff for up to three attempts; terminal validation failures and exhausted retries enter the visible dead-letter queue with their reason. A phone Retry creates a new request generation, and delayed phone commands cannot rewind newer Mac-owned execution state or results. Active program and equipment-discovery requests are deduplicated, and the iPhone Training screen gates the first program request behind goal, measurement, timeline, training-history, and facility-type setup. A generated program is shown as one latest proposal: open its detail screen to review the full plan, exact changes, Coach explanation, and accept/reject actions; text and image change requests create a replacement proposal rather than applying edits automatically. Each Mac status/result commit wakes an open sync session immediately and remains in the Mac outbox for the next phone foreground sync when no socket is available; the foreground sync keeps a short remote-activity settling window before closing.

The Mac dashboard shows only five recent jobs in a bounded scroll area. Clicking the Dock icon reopens the dashboard, closing its window leaves the menu-bar listener and worker running, and the explicit menu-bar Quit action is required to terminate the companion.

## HealthKit and Watch boundaries

The iPhone HealthKit import is optional. It imports a bounded recent range of steps, active energy, sleep, workouts, resting heart rate, HRV SDNN, body mass, and body-fat percentage. With an explicit user action, HealthCoach can also write an analyzed meal as energy/protein/carbohydrate/fat nutrition samples and a completed iPhone-only strength session as a `traditionalStrengthTraining` workout. A Watch-recorded workout remains the source of live heart rate and active energy, so the iPhone does not create a duplicate workout. Deterministic aggregation preserves units, freshness, source identity, manual-weight precedence for within-day summaries, sleep overlap handling, midnight/DST behavior, anchors, deleted-sample affected-day metadata, and tombstone replacements. Today surfaces the most recent HealthKit weight first and falls back to a manual weight only when no imported HealthKit weight exists; waist and body-fat values live in the occasional Body measurements check-in. Manual meals, measurements, goals, and training remain usable without iPhone HealthKit permission.
